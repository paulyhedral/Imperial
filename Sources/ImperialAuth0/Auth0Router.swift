...
public class Auth0Router : FederatedServiceRouter {
    ...
    public func authURL(_ request : Request) throws -> String {
        ...
        let stateValue = [ UInt8 ].random(count: 16).base64
        let auth = try Auth0Auth()
    ...
    //     }
//
    //     let body = Auth0CallbackBody(
    //       clientId: self.tokens.clientID, clientSecret: self.tokens.clientSecret, code: code)
//
    //     return try body.encode(to: request).flatMap(to: Response.self) { request in
    //       guard let url = URL(string: self.accessTokenURL) else {
    //         throw Abort(
    //           .internalServerError, reason: "Unable to convert String '\(self.accessTokenURL)' to URL")
    //       }
